...
public static class ExtractorCommandBuilder
{
    ...
    /// and drop to <see cref="ExtractorOptions.RunAsUid"/> <b>itself</b> (so the caps
    /// are enforced in-child before it touches the bytes); we pass them explicitly so
    /// the policy is one place and testable.
    /// </summary>
    public static IReadOnlyList<string> BuildArgs(ExtractorOptions options, string extension, string inputPath)
    {
