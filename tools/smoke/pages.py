...
class Thread:
    ...
    @property
    def activity(self) -> Locator:
        """A bot message's activity dropdown (thinking + tool cards behind one
        summary header). Open while the turn streams; collapses at message_end."""
        return self.root.locator(".activity")

    def open_activity(self, timeout: int = 15000) -> None:
        """Open the LAST message's activity dropdown so the tool cards inside are
        visible/clickable. Waits for the turn to settle first (streaming pins the
        dropdown open and message_end collapses it - a click landing mid-stream
        would be undone); a user click after that is sticky. Idempotent."""
        from playwright.sync_api import expect

        act = self.activity.last
        act.wait_for(state="attached", timeout=timeout)
        # the composer swaps send->stop while a turn streams; gone = settled
        expect(self.page.locator(".composer .stop")).to_have_count(0, timeout=timeout)
        if "open" not in (act.get_attribute("class") or ""):
            act.locator(".act-head").click()
            expect(act).to_have_class(re.compile(r"\bopen\b"))
