// fuzz.mjs - the automatic fuzzer entry point. Generates + mutates markdown,
// renders each input in an ISOLATED worker (lib/worker.mjs) under a hard per-input
...
// by its seed; the fuzzer also MINIMIZES each unique failure (lib/minimize.mjs)
...
//
// Usage:
//   node fuzz.mjs [--seed N] [-n ITERS | --time SECS] [--slow-ms MS]
//                 [--hang-ms MS] [--inline] [--no-mutate] [--quiet]
// Examples:
//   node fuzz.mjs -n 5000
//   node fuzz.mjs --time 30 --slow-ms 150
//   node fuzz.mjs --seed 12345        # replay a specific run
...
import { generate, generateInline } from "./lib/generators.mjs";
import { mutate, splice } from "./lib/mutators.mjs";
import { minimize } from "./lib/minimize.mjs";
...
function parseArgs(argv) {
  const a = { seed: null, iters: null, time: null, slowMs: 200, hangMs: 3000, inline: false, mutate: true, quiet: false };
...
class Runner {
  constructor(hangMs) { this.hangMs = hangMs; this.spawn(); }
  ...
  run(input, timeoutMs = this.hangMs) {
    return new Promise((resolve) => {
      let done = false;
      const onMsg = (m) => { if (done) return; done = true; clearTimeout(timer); this.worker.off("message", onMsg); resolve(m); };
...
function violationKind(detail) { return (String(detail).split(":")[0] || "").trim().toUpperCase(); }
function signature(input, res) {
  if (res.status === "throw") {
...
function predicateFor(runner, res, slowMs) {
...
// ---- input source -----------------------------------------------------------
function makeInput(rng, opts, corpus, recent) {
