// generators.ts - grammar-aware random markdown generators. Pure random bytes
// mostly produce paragraphs; a grammar that knows the renderer's constructs digs
// far deeper. Every generator takes an Rng (lib/rng.ts) so output is a pure
...
// ---- inline fragments -------------------------------------------------------
function word(rng: Rng): string { return rng.pick(WORDS); }
function runOf(rng: Rng, ch: string, max = 8): string { return ch.repeat(rng.range(1, max)); }

function inlineFrag(rng: Rng, depth: number): string {
...
function inlineLine(rng: Rng, depth = 2): string {
...
function evilCode(rng: Rng): string {
...
function table(rng: Rng): string {
  ...
  const cell = (): string => rng.chance(0.3) ? "`a|b`" : rng.chance(0.3) ? inlineFrag(rng, 0) : word(rng);
...
function list(rng: Rng, depth: number): string {
...
function blockquote(rng: Rng, depth: number): string {
...
function nestWall(rng: Rng): string {
  // near-cap and over-cap nesting to probe bounds/totality
  const ch = rng.pick([">", "- ", "* ", "+ "]);
  const n = rng.range(20, rng.chance(0.5) ? 40 : 120);
  return rng.times(n, (i: number) => ch.repeat(i + 1) + (ch === ">" ? "" : "") + "x").join("\n").slice(0, 4000);
}

export function block(rng: Rng, depth = 3): string {
  const kind = rng.weighted([
...
export function generate(rng: Rng, { maxBlocks = 8, depth = 3, maxLen = 8000 }: GenerateOptions = {}): string {
...
export function generateInline(rng: Rng): string { return inlineLine(rng, 4); }
