// minimize.ts - delta-debugging shrinker. A fuzzer's raw failing input is often
...
// predicate may be sync or async; we always await it (await on a non-thenable is a no-op).
export type Predicate = (input: string) => boolean | Promise<boolean>;
export interface MinimizeOptions { maxRounds?: number }

export async function minimize(input: string, predicate: Predicate, { maxRounds = 30 }: MinimizeOptions = {}): Promise<string> {
  ...
  while (changed && rounds++ < maxRounds) {
    ...
    if (!changed) {
      ...
      if (m) {
        // group 1 (.) always participates in a successful match, so m[1] is present.
