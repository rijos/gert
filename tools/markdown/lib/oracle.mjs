// oracle.mjs - the property oracle for the markdown renderer. Given the DOM
...
//
// Pure + dependency-free; operates on the shim nodes from lib/dom-shim.mjs (and,
...
export function elements(root) { return [...walk(root)].filter((n) => n.nodeType === ELEMENT_NODE); }
...
function isMml(el) { return el.namespaceURI && el.namespaceURI.includes("Math/MathML"); }
function inMath(el) { let p = el; while (p) { if (p.localName === "span" && (p.className || "").includes("md-math")) return true; p = p.parentNode; } return false; }

export function checkSecurity(root) {
  const viol = [];
...
export function checkBounds(root, srcLen, opts = {}) {
...
export function checkWellFormed(root) {
  const seen = new Set();
...
export function fingerprint(node) { return serialize(node); }
