// mutators.ts - structural + byte-level mutation operators. Mutation finds the
...
// A mutation operator: perturbs a string given the seeded Rng.
type Op = (rng: Rng, s: string) => string;
...
function clamp(s: string, max = 16000): string { return s.length > max ? s.slice(0, max) : s; }
...
export function mutate(rng: Rng, input: string, rounds = 0): string {
  let s = String(input ?? "");
  const k = rounds || rng.range(1, 4);
  // rng.int stays in [0, OPS.length); the operator at that index is always present.
  for (let i = 0; i < k; i++) s = OPS[rng.int(OPS.length)]!(rng, s);
  return clamp(s);
}

// splice(rng, a, b) -> a crossover of two seeds.
export function splice(rng: Rng, a: string, b: string): string {
