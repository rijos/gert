// proto/compare.mjs - A/B the per-character block scanner (proto/block-scan.mjs)
...
// the block-segmentation strategy. Run: node proto/compare.mjs
//
...
import { parseDocument, inline, importWww } from "../lib/render.mjs";
import { serialize } from "../lib/oracle.mjs";
import { parseBlocksPerChar } from "./block-scan.mjs";

const dom = await importWww("/lib/render/dom.js");
const ctx = { doc: globalThis.document, parseInline: inline.parseInline };

function renderAst(children) { return serialize(dom.render({ type: "document", children }, ctx)); }
function prod(src) { return renderAst(parseDocument(src).children); }
function proto(src) { return renderAst(parseBlocksPerChar(src, inline.parseInline)); }
...
console.log(disagree === 0
  ...
  : "=> Unexpected diff among SUPPORTED constructs; investigate proto/block-scan.mjs.");
