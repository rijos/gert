// proto/block-scan.mjs - EXPLORATION PROTOTYPE (not production).
//
...
// parseInline for inline content. proto/compare.mjs renders both this and the
...
const isSpace = (c) => c === " " || c === "\t";
function leadingSpaces(line) { let n = 0; while (n < line.length && line[n] === " ") n++; return n; }
function isBlank(line) { for (const c of line) if (!isSpace(c)) return false; return true; }
...
export function parseBlocksPerChar(src, parseInline, depth = 0) {
  const MAX_NEST = 32;
