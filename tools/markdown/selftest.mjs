// selftest.mjs - proves the FUZZER ITSELF works. A fuzzer that never reports a
...
// battery (no false positives). Run: node selftest.mjs
//
...
function runWith(moduleAbsPath, input, hangMs = 1500) {
  return new Promise((resolve) => {
    const w = new Worker(WORKER, { env: { ...process.env, GERT_RENDER_MODULE: moduleAbsPath } });
    let done = false;
    const finish = (r) => { if (done) return; done = true; clearTimeout(t); w.terminate(); resolve(r); };
    const t = setTimeout(() => finish({ status: "hang", ms: hangMs, detail: "timeout" }), hangMs);
    w.on("message", (m) => finish(m));
    w.on("error", (e) => finish({ status: "throw", ms: -1, detail: "worker error: " + e.message }));
    w.postMessage(input);
  });
}
...
const realUrl = JSON.stringify("file://" + join(HERE, "lib", "render.mjs"));
const domUrl = JSON.stringify("file://" + join(HERE, "lib", "dom-shim.mjs"));
...
function writeBuggy(name, code) { const p = join(TMP, name + ".mjs"); writeFileSync(p, code); return "file://" + p; }
...
const realMod = "file://" + join(HERE, "lib", "render.mjs");
const GOOD = [
  "# h *i* `c`", "- a\n- b", "> q\n> > r", "```python\ndef f():pass\n```",
