// check.ts - the MANUAL corpus runner + regression battery. Renders every
...
//
// Run: node check.ts   (exit 0 = all corpus cases satisfy every contract)

import { fileURLToPath } from "node:url";
import { dirname, join } from "node:path";
import { readdirSync, readFileSync } from "node:fs";
import { renderMarkdown } from "./lib/render.ts";
import { checkSecurity, checkBounds, checkWellFormed, serialize, elements } from "./lib/oracle.ts";

// Minimal structural views of the rendered shim nodes (lib/dom-shim.ts) this file reads.
// renderMarkdown / elements() are dynamic boundaries (the renderer returns `unknown`), so we
// narrow with a localized cast at each call site rather than threading a full DOM type.
interface FragmentLike { nodeType: number; }
interface ElementLike {
  localName: string;
  attributes?: { name: string; value: string }[];
  getAttribute(name: string): string | null;
}
// The renderer's facade is TOTAL: it normalizes any input internally, so the harness feeds it
// hostile non-string values. Its static signature is (string)=>unknown; cast at the boundary.
const render = renderMarkdown as (src: unknown) => unknown;
// Mirror the original `(e && e.message) || e` exactly (e is unknown after erasure): read an
// optional .message off the caught value, else fall back to the value itself. Behavior-identical.
function errMsg(e: unknown): unknown { return (e && (e as { message?: unknown }).message) || e; }

const HERE = dirname(fileURLToPath(import.meta.url));
const CORPUS = join(HERE, "corpus");

function contracts(src: string): string[] {
  const out: string[] = [];
  let frag: unknown;
  try { frag = render(src); }
  catch (e) { return [`THREW: ${errMsg(e)}`]; }
  if (!frag || (frag as FragmentLike).nodeType !== 11) out.push("did not return a DocumentFragment");
  ...
  let frag2: unknown;
  try { frag2 = render(src); } catch (e) { out.push("NONDET: 2nd render threw " + String(e)); }
...
function securityAssertions(src: string): string[] {
  const frag = render(src);
  // elements() walks the rendered tree; narrow (via unknown) to the element view this file reads.
  const els = elements(frag) as unknown as ElementLike[];
  const fails: string[] = [];
...
function totalityBattery(): string[] {
  ...
  for (const [label, input] of hostile) {
    try { const f = render(input); if (!f || (f as FragmentLike).nodeType !== 11) fails.push(`${label}: not a fragment`); }
    catch (e) { fails.push(`${label}: THREW ${errMsg(e)}`); }
  }
  return fails;
}
...
for (const f of files) {
  ...
  const els = (() => { try { return elements(render(src)).length; } catch { return -1; } })();
