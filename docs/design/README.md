...
- [configuration.md](configuration.md) - the configuration cascade
  (server -> user -> project -> conversation: provider choice + tool toggles cascade, **sampling
  does not** - it rides the selected provider), the project model, user settings, the chat
  provider catalog, and the user-facing data lifecycle. (Operator knobs:
...
- **Security review / audit:** [security.md](security.md) (boundaries + findings) ->
  [auth](auth.md) -> [storage-and-data section provisioning](storage-and-data.md#lazy-provisioning--migrations)
...
|---|---|---|
| Login, JWT validation, roles, tool grants | [auth.md](auth.md) | [security F11/F12](security.md#3-findings--remediations), [decisions section 3-4](decisions.md) |
| User/project folders, schemas, migrations, provisioning | [storage-and-data.md](storage-and-data.md) | [principles](principles.md), [configuration section 2](configuration.md#2-projects) |
