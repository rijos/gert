...
| S8 | Keyed turn parallelism with an atomic 409 gate (one combined change) | yes |
...
and S2a. *(S8 landed first, against today's code - see the note in its section below.)*
...
- **Depends:** -
- **Touches:** `Migrations/chat/002_heartbeat.sql` (`messages.heartbeat_at TEXT NULL`),
  ...
  `StaleTurnAfter >= 3 x HeartbeatInterval`), `IChatRepository` (touch method).
- **Design:** [chat-and-tools section detached turns](chat-and-tools.md#detached-turns) (the
  orphan rule), [turn-budgets section 2b](turn-budgets.md#2b-what-open-webui-does-surveyed-2026-06-07-open-webuiopen-webuimain)
...
- **Docs to update:** chat-and-tools (orphan rule), [installation section 9](../installation/configuration.md#9-gertturn---the-detached-turn-pipeline).
...
### S2a - Steering instead of 409 - backend ([turn-budgets section 4c](turn-budgets.md#4c-steering-instead-of-409---proposed-larger))
...
- **Race to close in tests:** steer lands after the final drain but before finalize -> the
  ...
  **or** double-sent.
- **Caveat to verify live:** tail injection x a thinking provider's reasoning replay
  (`chat_template_kwargs.preserve_thinking`) against vLLM 0.22 (the
  [turn-budgets section 6](turn-budgets.md#6-open-questions) open question) - a `Live_...` test in
...
- **Docs to update:** rest-api (the messages endpoint: 202-queued semantics replace the 409
  section ), chat-and-tools (plan/run phases), turn-budgets (mark 4c shipped).
...
- **Docs to update:** ui-components (section 7 map row), [spa-style-guide](spa-style-guide.md) only
...
- **Goal:** Embeddings stop riding the chat upstream's config. New **required** section -
  ...
  `EmbeddingModelId`/`EmbeddingDimensions` leave `Gert:OpenAI`. A second named `HttpClient`
  (own base address, key, resilience pipeline) backs `OpenAIEmbeddingClient` - today both
...
- **Touches:** `Gert.External` (`EmbeddingsOptions`, `ServiceCollectionExtensions` second
  client, `OpenAIEmbeddingClient.HttpClientName`), `appsettings.json` + `FakeE2E` profile,
  `tools/smoke` (mock URL into the new key; `serve-mock-vllm` leaves
  embeddings mocked), tests.
- **Docs to update:** [installation/configuration.md](../installation/configuration.md)
  (new `Gert:Embeddings` section; section 3 trimmed), [decisions section 1](decisions.md#1-embedding-model--dimension)
  (note the endpoint split), [operations section embeddings](operations.md#cross-cutting-concerns),
...
- **Touches:** docs only.
- **Docs to update:** [decisions.md](decisions.md) (new section 9 entry), [operations.md](operations.md)
  (deployment bullet), [tech-stack section engine portability](tech-stack.md#engine-portability)
  (one cross-link).
...
- **Docs to update:** context-compaction.md becomes the implemented spec folded into
  chat-and-tools + configuration; storage-and-data schema; installation section 9 knobs.
...
- **Docs to update:** rest-api (account & data), configuration section 5 (lifecycle table gains
...
> **yes Shipped** - see [decisions section 11](decisions.md#11-turn-execution---keyed-lanes-over-an-atomic-per-conversation-gate)
> (now settled). **Deviation from the Depends line below:** S8 landed
...
- **Goal:** Retire [decisions section 11](decisions.md#11-turn-execution---keyed-lanes-over-an-atomic-per-conversation-gate)'s
...
- **Docs to update:** decisions section 11 (mark remedied), chat-and-tools (detached turns -
  worker topology), storage-and-data (the new index), installation section 9 (the lane knob).
