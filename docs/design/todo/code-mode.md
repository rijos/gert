...
- **Gate each call on `job.ToolIds`** (the *offered* set = entitlement intersect conversation-toggle intersect
  request intersect model-cap) **intersect `CodeModeExposable`**, stricter than the model's own outer
...
- **`TurnRunner`**: extract the per-call block (emit Running -> execute -> emit Result -> persist
  ...
  `job.ToolIds intersect CodeModeExposable`. Inner calls surface as **sibling cards** (reuse the
...
- Unit: the `ToolIds intersect CodeModeExposable` gate + no-recursion; **the egress-free meta-test**
