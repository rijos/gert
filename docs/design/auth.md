...
public sealed class UserContext(IHttpContextAccessor http, ToolRegistry registry)
{
    ...
    public IReadOnlySet<string> AllowedTools
    {
        get
        {
            ...
            return registry.Normalize(raw);                 // delimited scope string intersect registry
        }
    }
...
capability comes from the token or not at all. (The claim is a **scope string**; a
...
Reaching the fail-closed floor is correct but easy to misread from the outside - a user who
toggled "web search" on watches the model improvise pseudo-call syntax instead of searching.
When every requested tool was dropped *by the entitlement specifically*, the planner logs a
warning naming the dropped count and the token's grant count, so a missing/misconfigured
