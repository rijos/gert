...
> **One-line model:** a **project is its own scope** - its own conversations and documents,
> fully isolated from every other project. **"Default" is just the project you start
...
| **User** | theme, UI language, default reply language, default provider, default tools | `user.db` `settings` row ([storage-and-data section user.db](storage-and-data.md#userdb)) | the user |
...
block, and the data stores that are *only* ever opened for that project - conversations and
documents (RAG). Nothing crosses a project boundary. A user can have many; they
...
### 2.3 Instructions
A project carries an optional **instructions** block - a small, always-injected piece of custom
system prompt. It lives on the project's registry row in `user.db`, length-bounded
([section 6](#6-what-is-not-configurable) notes the cap). This is the cheap, deterministic
"always know this" context that rides every turn in that project.
...
/data/users/{key}/
  ...
  │                         #   languages, defaults), and the PROJECT REGISTRY -
...
### 3.3 Defaults
Default **provider** (the model picker's selection) and default **tools** (each capped by the
user's `gert_tools` JWT entitlement -
[auth](auth.md#tool-entitlements-allowed-tools-in-the-jwt)). These seed every new project and
conversation unless overridden.
...
The settings/projects/lifecycle endpoints, in shape form - the full contracts live
in [rest-api](rest-api.md), the SPA surface in [section 8](#8-the-spa-surface).

```
# user settings
GET    /api/settings                       # theme, languages, defaults
PUT    /api/settings
...
- **Project settings** - name, instructions, defaults, "forget documents".
