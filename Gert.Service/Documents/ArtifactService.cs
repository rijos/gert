...
/// (rest-api.md § artifacts). Read-only; artifacts are produced by the chat loop
/// (U7b). Scoped to the current user via <see cref="IUserContext"/>.
/// </summary>
public sealed class ArtifactService : IArtifactService
{
    ...
    public ArtifactService(IDatabaseProvider databases, IUserContext user)
    {
