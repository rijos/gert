...
/// <c>{pid}</c> (configuration.md §2.5). These feed directly into a filesystem
/// path, so they are validated <b>before</b> any path-join/<c>rm -rf</c>. Exposed
/// as a static seam the API controllers (U9b) and the Console call so both hosts
/// share one rule; they return the same <see cref="ValidationResult"/> shape the
...
public static class RouteParamValidation
{
    ...
    /// (configuration.md §2.5). Defence-in-depth — isolation is structural, but a
