// state/knowledge.js — documents + per-doc ingest status, plus the
// "use in this chat" switch. Keyed list via van-x so one doc's status pill
// re-renders alone. No DOM, no fetch.
import van from "van";
import { reactive } from "van-x";

export const documents = reactive([]); // [{ id, name, size, chunk_count, status, error, progress }]
export const useInChat = van.state(true);
...
export const toggleUseInChat = () => (useInChat.val = !useInChat.val);
