...
/// (<c>ux_messages_streaming</c>, decisions §11) under genuine cross-connection
...
public class TurnPlannerGateTests
{
    ...
    public async Task Concurrent_plans_for_one_conversation_yield_exactly_one_streaming_placeholder()
    {
        ...
        for (var round = 0; round < 20; round++)
        {
            ...
            var t1 = Task.Run(async () => { await start.WaitAsync(); return await PlanOnce(); });
            var t2 = Task.Run(async () => { await start.WaitAsync(); return await PlanOnce(); });
            start.Release(2);
            ...
            messages.Count(m => m.Status == MessageStatus.Streaming).Should().Be(1,
