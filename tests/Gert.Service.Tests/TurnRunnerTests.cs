...
public sealed class TurnRunnerTests
{
    ...
    private TurnRunner NewRunner(
        ...
        new(_chatProvider, new FixedChatClientFactory(model), _bus, tools ?? [], Options.Create(options ?? new TurnOptions()),
    ...
    public async Task Capped_round_refuses_calls_with_synthetic_results_keeping_the_wire_format()
    {
        ...
        // budget message for the card's error line (turn-budgets.md section 6).
    ...
    public async Task Per_turn_search_budget_refuses_searches_past_the_cap()
    {
        ...
        await NewRunner(model, [search], new TurnOptions { MaxSearchCallsPerTurn = 2 })
            .RunAsync(NewJob("research this", [search]));

        // Two searches execute, the rest refuse with a synthetic budget result
        // the model can read - the turn still finalises complete.
        search.Executions.Should().Be(2);
        _toolRows.Count(r => r.Status == ToolCallStatus.Done).Should().Be(2);
        _toolRows.Count(r => r.Status == ToolCallStatus.Error).Should().Be(2);
    ...
    public async Task Disabled_search_budget_lets_every_search_run()
    {
        ...
        await NewRunner(model, [search], new TurnOptions { MaxSearchCallsPerTurn = 0 })
            .RunAsync(NewJob("research this", [search]));

        search.Executions.Should().Be(4);
        _toolRows.Should().OnlyContain(r => r.Status == ToolCallStatus.Done);
    }

    [Fact]
    public async Task Per_round_max_tokens_is_the_configured_bound_else_unset()
    {
        // max_completion_tokens is no longer a per-conversation override - every
        // round carries the configured TurnOptions.MaxTokensPerRound, or nothing
        // when it is disabled (0), leaving the provider's own default to apply.
        var model = new RequestCapturingModel();

        await NewRunner(model, options: new TurnOptions { MaxTokensPerRound = 100 }).RunAsync(NewJob("hello"));
        await NewRunner(model, options: new TurnOptions { MaxTokensPerRound = 0 }).RunAsync(NewJob("hello"));
    ...
    /// drives the per-turn search budget.
    /// </summary>
