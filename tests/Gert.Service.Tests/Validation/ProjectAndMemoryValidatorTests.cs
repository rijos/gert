...
/// conversation / generation-params validators (testing.md section 5). One file
/// keeps the related cases together; each validator is the production registration.
/// </summary>
public sealed class ProjectAndMemoryValidatorTests
{
    ...
    public void CreateMemory_requires_safe_title_and_content()
    {
        ...
        v.TestValidate(new CreateMemoryRequest { Title = "", Content = "Body" })
    ...
    [Fact]
    public void GenerationParams_bounds_are_enforced()
    {
        var v = _sp.Validator<GenerationParams>();
        v.TestValidate(new GenerationParams { Temperature = 0.7, TopP = 0.9, MaxTokens = 1000 })
            .ShouldNotHaveAnyValidationErrors();
        v.TestValidate(new GenerationParams { Temperature = -0.1 })
            .ShouldHaveValidationErrorFor(p => p.Temperature);
        v.TestValidate(new GenerationParams { Temperature = 2.1 })
            .ShouldHaveValidationErrorFor(p => p.Temperature);
        v.TestValidate(new GenerationParams { TopP = 1.5 })
            .ShouldHaveValidationErrorFor(p => p.TopP);
        v.TestValidate(new GenerationParams { MaxTokens = 0 })
            .ShouldHaveValidationErrorFor(p => p.MaxTokens);
    }
