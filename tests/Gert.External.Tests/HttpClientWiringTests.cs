...
/// (dotnet-style-guide.md §9: a streaming client's <c>HttpClient.Timeout</c> must not cap
...
/// turn-budgets.md §4a) with an options-bound pre-stream pipeline; embeddings = its own
...
public sealed class HttpClientWiringTests
{
    ...
    public void Chat_client_timeout_is_infinite_so_the_turn_budget_owns_the_stream()
    {
        ...
        var client = provider.GetRequiredService<IHttpClientFactory>()
            .CreateClient(VllmChatModelClient.HttpClientName);
    ...
    {
        VllmEmbeddingClient.HttpClientName.Should().NotBe(
            VllmChatModelClient.HttpClientName,
            "the buffered embeddings path must not inherit the streaming client's infinite timeout");

        using var provider = Provider();
        var client = provider.GetRequiredService<IHttpClientFactory>()
            .CreateClient(VllmEmbeddingClient.HttpClientName);
        var pipeline = Pipeline(provider, VllmEmbeddingClient.HttpClientName);
    ...
    [Theory]
    [InlineData(VllmChatModelClient.HttpClientName)]
    [InlineData(VllmEmbeddingClient.HttpClientName)]
    public void Vllm_resilience_pipeline_is_bound_to_the_options_not_stock_defaults(string clientName)
    {
        using var provider = Provider(new Dictionary<string, string?>
        {
            ["Gert:Vllm:RequestTimeoutSeconds"] = "42",
            ["Gert:Vllm:RetryCount"] = "4",
        });
        ...
        // Total = attempt × (retries + 1) + backoff slack 3·(2^retries − 1) s:
        ...
        // The standard validator demands SamplingDuration ≥ 2 × attempt.
        pipeline.CircuitBreaker.SamplingDuration.Should().Be(TimeSpan.FromSeconds(84));
    }

    [Fact]
    public void Vllm_retry_count_zero_is_accepted_and_disables_retries()
    {
        using var provider = Provider(new Dictionary<string, string?>
        {
            ["Gert:Vllm:RetryCount"] = "0",
        });
        ...
        var pipeline = Pipeline(provider, VllmChatModelClient.HttpClientName);
    ...
    public void Monty_defaults_satisfy_the_wall_clock_relation()
    {
        ...
        monty.RequestTimeoutSeconds.Should().BeGreaterThan(
            provider.GetRequiredService<IOptions<SandboxOptions>>().Value.WallClockSeconds,
