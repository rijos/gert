...
{
    private static MontySandbox NewSandbox(StubHttpMessageHandler handler, SandboxOptions? options = null)
    {
        var http = new HttpClient(handler) { BaseAddress = new Uri("http://monty.test") };
        return new MontySandbox(http, Options.Create(options ?? new SandboxOptions()), NullLogger<MontySandbox>.Instance);
    }
