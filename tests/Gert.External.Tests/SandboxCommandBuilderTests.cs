...
public sealed class SandboxCommandBuilderTests
{
    ...
    {
        var config = SandboxCommandBuilder.BuildRuntimeConfig(new SandboxOptions(), "print(1)");
    ...
    public void BuildRuntimeConfig_EgressOptIn_FlowsThrough()
    {
        var config = SandboxCommandBuilder.BuildRuntimeConfig(
            new SandboxOptions { EgressEnabled = true }, "x");
    ...
    {
        var args = SandboxCommandBuilder.BuildRunscArgs(new SandboxOptions(), "cid", "/tmp/bundle");
    ...
    public void BuildRunscArgs_EgressOn_UsesHostNetwork()
    {
        var args = SandboxCommandBuilder.BuildRunscArgs(
            new SandboxOptions { EgressEnabled = true }, "cid", "/tmp/b");
    ...
    {
        var options = new SandboxOptions { RunscPath = "/nonexistent/runsc-xyz" };
        GVisorSandbox.IsAvailable(options).Should().BeFalse();
    }
    ...
    public async Task RunPythonAsync_WhenUnavailable_ReturnsCleanResult_NotThrow()
    {
        var sandbox = new GVisorSandbox(
            Microsoft.Extensions.Options.Options.Create(new SandboxOptions { RunscPath = "/nonexistent/runsc-xyz" }),
    ...
    // Keep ISandbox referenced so the test asserts against the port contract.
    [Fact]
    public void GVisorSandbox_ImplementsPort()
    {
        typeof(ISandbox).IsAssignableFrom(typeof(GVisorSandbox)).Should().BeTrue();
    }
}
