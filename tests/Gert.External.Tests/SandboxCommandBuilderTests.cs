...
public sealed class SandboxCommandBuilderTests
{
    ...
    {
        var config = SandboxCommandBuilder.BuildRuntimeConfig(new PythonSandboxOptions(), "print(1)");
    ...
    public void BuildRuntimeConfig_EgressOptIn_FlowsThrough()
    {
        var config = SandboxCommandBuilder.BuildRuntimeConfig(
            new PythonSandboxOptions { EgressEnabled = true }, "x");
    ...
    {
        var args = SandboxCommandBuilder.BuildRunscArgs(new PythonSandboxOptions(), "cid", "/tmp/bundle");
    ...
    public void BuildRunscArgs_EgressOn_UsesHostNetwork()
    {
        var args = SandboxCommandBuilder.BuildRunscArgs(
            new PythonSandboxOptions { EgressEnabled = true }, "cid", "/tmp/b");
    ...
    {
        var options = new PythonSandboxOptions { RunscPath = "/nonexistent/runsc-xyz" };
    ...
    public async Task RunPythonAsync_WhenUnavailable_ReturnsCleanResult_NotThrow()
    {
        var sandbox = new GVisorSandbox(
            Microsoft.Extensions.Options.Options.Create(new PythonSandboxOptions { RunscPath = "/nonexistent/runsc-xyz" }),
