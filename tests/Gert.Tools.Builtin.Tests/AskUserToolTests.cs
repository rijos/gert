...
/// <summary>
/// The ask_user tool (chat-and-tools.md section Ask the user): argument
/// validation as model-correctable errors, the multi-question (tabbed) payload,
/// the emit ordering (question_asked before the wait, question_answered before
/// the result), the graceful timeout result, the deadline budget math, and key
...
public sealed class AskUserToolTests
{
    ...
    private static QuestionPayload OneOpen =>
    ...
    public async Task More_than_four_questions_are_refused()
    {
        ...
        var result = await NewTool().ExecuteAsync(Invocation($$"""{"questions":[{{five}}]}"""));
    ...
    public async Task Too_many_or_too_long_options_are_refused()
    {
        ...
        var many = await NewTool().ExecuteAsync(
            Invocation($$"""{"questions":[{"question":"q","options":[{{nine}}]}]}"""));
        ...
        var tooLong = await NewTool().ExecuteAsync(
            Invocation($$"""{"questions":[{"question":"q","options":["{{longOption}}"]}]}"""));
    ...
    public async Task A_too_long_header_is_refused()
    {
        ...
        var result = await NewTool().ExecuteAsync(
            Invocation($$"""{"questions":[{"question":"q","header":"{{longHeader}}"}]}"""));
    ...
    {
        var result = await NewTool().ExecuteAsync(
            Invocation("""{"questions":[{"question":"q"}]}""", conversationId: null));
    ...
    {
        var result = await NewTool().ExecuteAsync(
            Invocation("""{"questions":[{"question":"q"}]}""", withEmit: false));
    ...
    public async Task An_answer_resolves_the_call_with_both_events_in_order()
    {
        var task = NewTool().ExecuteAsync(
            Invocation("""{"questions":[{"question":"Which color?","options":["red","blue"]}]}"""));
        ...
        only.AllowFreeText.Should().BeFalse("options without allow_free_text default to closed");
        ...
        using var next = _registry.Open(Key, OneOpen);
    }

    [Fact]
    public async Task Several_questions_are_tabs_paired_with_their_answers_in_order()
    {
        var task = NewTool().ExecuteAsync(Invocation(
            """
            {"questions":[
        ...
        asked.Questions[1].Header.Should().Be("Notes");
        asked.Questions[1].AllowFreeText.Should().BeTrue("no options defaults open");
        asked.Questions[2].AllowFreeText.Should().BeTrue("explicit allow_free_text alongside options");
    ...
    public async Task A_question_without_options_defaults_to_free_text()
    {
        ...
        await NewTool(options).ExecuteAsync(Invocation("""{"questions":[{"question":"Anything?"}]}"""));

        AskedEvent()!.Questions.Single().AllowFreeText.Should().BeTrue();
    }
    ...
    public async Task A_timeout_is_a_graceful_no_response_result_not_an_error()
    {
        ...
        var result = await NewTool(options).ExecuteAsync(
            Invocation("""{"questions":[{"question":"q"}]}"""));
    ...
    public async Task A_turn_cancel_unwinds_the_wait_and_releases_the_key()
    {
        ...
        var task = NewTool().ExecuteAsync(
            Invocation("""{"questions":[{"question":"q"}]}"""), cts.Token);
    ...
    {
        using var occupied = _registry.Open(
            Key, new QuestionPayload([new QuestionItem("first?", null, [], AllowFreeText: true)]));

        var result = await NewTool().ExecuteAsync(
            Invocation("""{"questions":[{"question":"second?"}]}"""));
