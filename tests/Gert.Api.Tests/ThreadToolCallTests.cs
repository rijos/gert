...
public sealed class ThreadToolCallTests
{
    ...
    [Theory]
    [InlineData("make_artifact")]
    [InlineData("edit_artifact")]
    [InlineData("read_artifact")]
    public void Artifact_call_carries_the_file_name_as_its_query(string kind)
    {
        // The reload twin of the live event's request?.name fallback - the SPA
        // joins the message's artifact chips and activity summary on this.
        var card = ThreadToolCall.From(
            Call(kind, requestJson: """{"name":"index.html","kind":"html"}"""),
            []);

        card.Query.Should().Be("index.html");
    }
