...
///   <item><c>user</c>    - groups <c>gert-users</c>, <c>gert_tools</c> <c>rag search make_artifact edit_artifact read_artifact ask_user fetch</c>.</item>
...
public sealed class TestTokens : IDisposable
{
    ...
    public string MintUser(params (string Type, string Value)[] extraClaims) =>
        Mint("dev-user", Issuer, ["gert-users"], "rag search make_artifact edit_artifact read_artifact ask_user fetch", extraClaims: extraClaims);
