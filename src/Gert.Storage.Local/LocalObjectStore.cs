...
public sealed class LocalObjectStore : IObjectStore
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // Pure recursive delete. Any chat.db/rag.db sharing this tree (the default
        // single-root deployment) were already released + removed by the database engines
        // (the service deletes the db halves first); with a separate engine DataRoot they
        // are not in this tree at all. Either way this never unlinks an open file.
