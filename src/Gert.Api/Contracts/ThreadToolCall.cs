...
public sealed record ThreadToolCall
{
    ...
    private static string? QueryOf(string kind, JsonElement? request) => kind switch
    {
        ...
        // the artifact tools' request names the file - the reload twin of the
        // live event's request?.name fallback (the SPA joins artifact chips and
        // the activity summary on it).
        "make_artifact" or "edit_artifact" or "read_artifact" => GetString(request, "name"),
