...
// the cast names exactly the four tags we destructure so noUncheckedIndexedAccess does not
// widen them to `| undefined` (runtime is byte-identical after type elision).
const { svg, path, circle, rect } = van.tags("http://www.w3.org/2000/svg") as Readonly<{
  svg: TagFunc<Element>;
  path: TagFunc<Element>;
  circle: TagFunc<Element>;
  rect: TagFunc<Element>;
}>;

// each entry: () => array of child nodes for a 0 0 24 24 viewBox. Typed as a string-keyed
...
const GLYPHS: Record<string, () => Element[]> = {
  ...
  // shield - the admin-area affordance
  shield: () => [
  ...
  // counter-clockwise arrow - the message retry action
  retry: () => [
  ...
  // four-point spark - the activity (thinking + tools) summary mark
  sparkle: () => [
    path({ d: "M12 3c.7 4.9 2.6 6.8 7.5 7.5-4.9.7-6.8 2.6-7.5 7.5-.7-4.9-2.6-6.8-7.5-7.5C9.4 9.8 11.3 7.9 12 3z", "stroke-linejoin": "round" }),
    path({ d: "M19 15.5c.3 2 1.1 2.8 3 3-1.9.3-2.7 1.1-3 3-.3-1.9-1.1-2.7-3-3 1.9-.2 2.7-1 3-3z", "stroke-linejoin": "round" }),
  ],
...
// Options for Icon(). All optional; the body falls back per-field, so no undefined is ever
// assigned to a tag prop (exactOptionalPropertyTypes-safe).
