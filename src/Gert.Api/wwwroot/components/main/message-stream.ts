...
const EmptyHero = () =>
  div({ class: "thread-empty" },
    ...
    div({ class: "te-chips" },
      ...STARTERS.map((text) =>
        button({ class: "te-chip", onclick: () => (chat.draft.val = text) }, text),
      ),
    ),
  );
...
export const MessageStream = component({
  ...
  css: `
    ...
    .thread-empty .te-mark {
      width: 34px;
      height: 34px;
      opacity: .9;
    }
    .thread-empty .te-mark svg {
      width: 100%;
      height: 100%;
    }
    .thread-empty h2 {
    ...
    .jump-present {
      ...
      transition: var(--t-fast);
      z-index: 2;
      animation: rise .25s var(--ease) backwards;
    }
    ...
    .jump-present:hover {
      border-color: var(--coral);
      color: var(--coral-deep);
      background: linear-gradient(var(--coral-soft), var(--coral-soft)) var(--surface);
    }
  `,
  // away = the reader scrolled well clear of the bottom - drives the "jump to
  // present" pill (a state, not the plain flag, because it drives DOM). The
  // scroll node, the `pinned` flag, and the two stream-scoped derives stay in
  // view (section 12: a derive that prunes with the component lives in view).
  setup: () => {
  ...
  // real 3-arg shape at these calls - runtime is byte-identical.
  const deriveScoped = van.derive as (f: () => void, s: undefined, dom: Element) => unknown;
