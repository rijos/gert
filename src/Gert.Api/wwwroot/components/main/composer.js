...
export const Composer = component({
  ...
  css: `
    ...
    .composer{max-width:760px; margin:0 auto; background:var(--surface); border:1px solid var(--line); border-radius:var(--r-lg); padding:13px 15px 11px; box-shadow:var(--lift); transition:var(--t-fast);}
    ...
    .composer textarea{width:100%; border:none; background:none; resize:none; outline:none; font-family:var(--sans); font-size:var(--fs-base); color:var(--ink); line-height:var(--lh-ui); min-height:24px;}
    .composer textarea::placeholder{color:var(--ink-3);}
    .crow{display:flex; align-items:center; gap:9px; margin-top:9px;}
    .cbtn{background:none; border:1px solid var(--line); border-radius:var(--r-sm); padding:6px 10px; cursor:pointer; color:var(--ink-2); font-family:var(--sans); font-size:var(--fs-sm); font-weight:500; display:flex; align-items:center; gap:6px; transition:var(--t-fast);}
    ...
    .cbtn.toggle.on{border-color:var(--coral-line); color:var(--coral-deep); background:var(--coral-soft);}
    /* right cluster: context ring + send/stop pinned to the corner */
    .cright{margin-left:auto; display:flex; align-items:center; gap:9px;}
    .send{width:36px; height:36px; border-radius:10px; border:1px solid var(--coral-line); background:var(--coral-soft); color:var(--coral-deep); cursor:pointer; display:grid; place-items:center; transition:var(--t-fast);}
    /* colour-only hover: a transform here moved the button under the cursor */
    .send:hover{background:var(--coral-deep); border-color:var(--coral-deep); color:var(--on-accent);}
    /* empty textbox -> inert grey button (no faded-accent look) */
    .send:disabled{background:var(--surface-2); border-color:var(--line); color:var(--ink-3); cursor:default;}
    .send:disabled:hover{background:var(--surface-2);}
    .send svg{width:17px; height:17px;}
    /* shown while the turn streams - click to detach (stop) */
    .stop{width:36px; height:36px; border-radius:10px; border:none; background:var(--coral-deep); color:var(--on-accent); cursor:pointer; display:grid; place-items:center; transition:var(--t-fast); animation:pulse 1.4s ease-in-out infinite;}
    .stop:hover{background:var(--coral);}
    ...
    .att-x{position:absolute; top:3px; right:3px; width:18px; height:18px; border-radius:50%; border:none; background:var(--scrim-chip); color:var(--on-scrim); font-size:var(--fs-sm); line-height:1; cursor:pointer; display:grid; place-items:center; padding:0; transition:var(--t-fast);}
  ...
  view: () => {
    ...
    // (macOS/iOS); Ctrl+Enter elsewhere
        ...
        div(
          ...
          div(
            ...
            () =>
              chat.streaming.val
                ? button(
                    { class: "stop", title: t("Stop"), onclick: chatSvc.stop },
