...
export const Composer = component({
  ...
  css: `
    ...
    .send:hover{background:var(--coral-deep); border-color:var(--coral-deep); color:var(--on-accent); transform:translateY(-1px);}
  ...
  view: () => {
    ...
    // the send chord reads ⌘↵ only where ⌘ exists (macOS/iOS); Ctrl+↵ elsewhere
    ...
    const ta = textarea({
      rows: 1,
      placeholder: `Message Gert…  ${isMac ? "⌘↵" : "Ctrl+↵"} to send`,
      oninput: (e) => {
        autogrow(e.target);
    ...
    // `wrap` so the derive is pruned with the component (§12).
    van.derive(
      () => {
