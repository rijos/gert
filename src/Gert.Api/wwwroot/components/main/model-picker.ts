...
import type { Model } from "../../state/models.js";

const { div, button, span } = van.tags;

// A catalog row may also carry a human `endpoint` label (the wire includes it
// for multi-endpoint deployments); it is not part of the core Model contract,
// so widen locally rather than re-declare the store row.
  ...
  button({ class: () => "m-item" + (models.selectedId.val === m.id ? " sel" : ""), type: "button", "aria-current": () => (models.selectedId.val === m.id ? "true" : "false"), onclick: () => models.select(m.id) },
    div({ class: "m-top" },
      span({ class: "m-name" },
        ...
        m.fast
          ? span({ style: "color:var(--ink-3);font-weight:400;font-size:var(--fs-xs)" }, " - fast")
          : null,
      ),
    ),
    div({ class: "m-id" }, (m.id || "") + (m.endpoint ? " - " + m.endpoint : "")),
    div({ class: "badges" },
      ...(m.capabilities || []).map((c: string) => Badge({ label: c, cap: true })),
      m.context != null
