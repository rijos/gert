...
// The reactive `card.question` object services/chat.js folds onto an ask_user
// tool card (its private ToolQuestion shape). The card mutates `posting`/
// `expired` in place, so the fields stay writable.
interface Question {
  questionId: string;
  text: string;
  options: string[];
      ...
      q.options.length
        ? div({ class: "qopts" },
            ...q.options.map((opt: string) =>
              button({ class: () => "qopt" + (q.answered && q.answer === opt ? " chosen" : ""), type: "button", disabled: inert, onclick: () => submit(opt) }, opt),
            ),
          )
        : "",
      () =>
        q.allowFreeText && !q.answered && !q.expired
          ? div(
              ...
              input({
                ...
                value: draft,
                oninput: (e: Event) => (draft.val = (e.target as HTMLInputElement).value),
                onkeydown: (e: KeyboardEvent) => {
                  if (e.key === "Enter") submit(draft.val);
                },
              }),
              button({ class: "qsend", type: "button", disabled: () => q.posting, onclick: () => submit(draft.val) }, "Answer"),
            )
          : "",
