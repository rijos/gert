...
const TI_LABEL = { md: "md", html: "<>", svg: "svg", py: ".py" };

const TypeIcon = (a) => span({ class: "ti " + a.kind }, TI_LABEL[a.kind] || "?");
...
export const ArtifactTabs = component({
  name: "artifact-tabs",
  css: `
    .ctabs-wrap{display:flex; align-items:center; gap:4px; flex:1; min-width:0; position:relative;}
    ...
    .ctab .ti,.ca-item .ti{min-width:13px; height:13px; padding:0 2px; flex:none; border-radius:3px; display:grid; place-items:center; font-size:7.5px; font-weight:700; color:var(--on-chip); letter-spacing:-.02em;}
    .ti.md{background:var(--type-md);} .ti.html{background:var(--coral);} .ti.svg{background:var(--amber);} .ti.py{background:var(--type-py);}
    ...
    .ctab-all{display:flex; align-items:center; gap:4px; flex:none; padding:5px 7px; background:none; border:1px solid var(--line); border-radius:7px; color:var(--ink-2); font-family:var(--mono); font-size:var(--fs-2xs); cursor:pointer; transition:var(--t-fast);}
    .ctab-all:hover{border-color:var(--coral); color:var(--coral-deep);}
    .ctab-all .chev{transition:var(--t-slow) var(--ease);}
    .ctabs-wrap.open .chev{transform:rotate(180deg);}
    .ctabs-wrap .menu{width:232px; max-height:50vh; overflow-y:auto;}
    .ctabs-wrap.open .menu{opacity:1; transform:none; pointer-events:auto;}
    .ca-item{display:flex; align-items:center; gap:7px; padding:var(--sp-2) var(--sp-3); border-radius:var(--r-sm); cursor:pointer; transition:var(--t-fast); font-family:var(--mono); font-size:var(--fs-xs); color:var(--ink-2);}
    .ca-item:hover{background:var(--surface-2); color:var(--ink);}
    .ca-item.sel{background:var(--coral-soft); color:var(--coral-deep);}
    .ca-item .ca-name{flex:1; min-width:0; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;}
  `,
  view: () => {
    // -- logic -----------------------------------
