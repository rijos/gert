...
import type { WireMemoryEntry } from "../../services/wire.js";
...
export const MemoryList = component({
  ...
  css: `
    ...
    .mem-row:hover .mem-del {
      display: flex;
    }

    .mem-row .mem-del:hover {
      color: var(--brick);
      background: var(--surface-2);
    }
  ...
  setup: (): MemoryListState => {
    ...
    const remove = (m: MemoryEntry) =>
      attempt(async () => {
        await memorySvc.remove(m.id);
        refresh();
      }, "Couldn't delete the memory");

    return { entries, refresh, remove, promptAdd };
  },
  view: ({ entries, refresh, remove, promptAdd }: MemoryListState) => {
    const wrap = div(
      { class: "memlist" },
      div(
        { class: "mem-h" },
