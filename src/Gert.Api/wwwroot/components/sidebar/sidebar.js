...
export const Sidebar = component({
  ...
  css: `
    ...
    .newchat{margin:4px 16px 14px; padding:11px 14px; border:1px solid var(--coral-deep); background:var(--coral-deep);
      border-radius:var(--r); font-family:var(--sans); font-weight:600; font-size:var(--fs-md); color:var(--on-accent);
      display:flex; align-items:center; justify-content:center; gap:9px; cursor:pointer; transition:var(--t-fast);
      width:calc(100% - 32px); box-shadow:var(--lift);}
    /* colour-only hover: a transform here moved the button under the cursor */
    .newchat:hover{background:var(--coral); border-color:var(--coral);}
    .newchat svg{width:15px; height:15px;}

    /* quiet full-screen-search entries under the call to action */
    .side-search{display:flex; gap:6px; margin:0 16px 12px;}
    .side-search button{flex:1; display:flex; align-items:center; justify-content:center; gap:7px; padding:7px 10px; background:none; border:1px solid var(--line); border-radius:var(--r-sm); font-family:var(--sans); font-size:var(--fs-xs); font-weight:500; color:var(--ink-2); cursor:pointer; transition:var(--t-fast);}
    .side-search button:hover{border-color:var(--coral); color:var(--coral-deep); background:var(--coral-soft);}
  ...
  view: () =>
    aside(
      ...
      div(
        { class: "side-search" },
        button(
          { onclick: () => openSearch("chats"), title: t("Search all chats") },
          Icon("search", { size: 12, strokeWidth: 2.2 }),
          t("Chats"),
        ),
        button(
          { onclick: () => openSearch("projects"), title: t("Search all projects") },
          Icon("search", { size: 12, strokeWidth: 2.2 }),
          t("Projects"),
        ),
      ),
