...
export const ProjectPicker = component({
  ...
  css: `
    ...
    .p-row{display:flex; align-items:center; gap:6px; min-width:0;}
    .p-row .p-name{flex:1; min-width:0; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;}
    .p-act{opacity:0; display:flex; align-items:center; justify-content:center; width:22px; height:22px; flex:none; background:none; border:none; border-radius:5px; color:var(--ink-3); cursor:pointer; padding:0; transition:var(--t-fast);}
    .dd-item:hover .p-act,.p-act:focus-visible{opacity:1;}
    .p-act:hover{color:var(--coral-deep); background:var(--coral-soft);}
    .p-act.danger:hover{color:var(--brick); background:var(--surface-2);}
  `,
  view: () => {
    const promptNew = () => {
          ...
          button(
            {
              class: "p-act",
              title: t("Rename project"),
              "aria-label": `Rename project ${item.label}`,
              onclick: (e) => {
                e.stopPropagation();
                promptRename(item);
              },
