...
// phishing form to an attacker. Scripts (HTML only) still run inline for
// fidelity, they just have nowhere to phone home.
//
 ...
 */
export const artifactSrcdoc = (content, { allowScripts = false } = {}) => {
  ...
  if (/<head[^>]*>/i.test(src)) return src.replace(/<head[^>]*>/i, (m) => m + meta);
  // A document with <html> but no <head>: give it one, meta first.
  if (/<html[^>]*>/i.test(src))
    return src.replace(/<html[^>]*>/i, (m) => `${m}<head>${meta}</head>`);
