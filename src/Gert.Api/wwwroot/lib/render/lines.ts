...
function isFenceClose(line: string, ch: string, len: number) {
  const m = /^( {0,3})(`{3,}|~{3,})[ \t]*$/.exec(line);
...
function splitRowCells(row: string) {
  let s = row.trim();
  if (s.startsWith("|")) s = s.slice(1);
  if (/(^|[^\\])\|$/.test(s)) s = s.replace(/\|$/, "");
  const cells: string[] = []; let buf = "", ticks = 0;
  for (let i = 0; i < s.length; i++) {
    // i < s.length holds (loop condition), so s[i] is a defined char.
    const ch = s[i]!;
    if (ticks === 0) {
...
function cellAlignment(c: string) {
  const t = c.trim(); const l = t.startsWith(":"), r = t.endsWith(":");
  return l && r ? "center" : r ? "right" : l ? "left" : null;
}
interface ListMarker { ordered: boolean; start: number; contentCol: number; rest: string; }
...
type LineTest = (line: string, look: string | undefined, depth: number) => Caps | null;

export const LINE_KINDS = Object.freeze([
  Object.freeze({ kind: LineKind.FENCE, test: ((line) => matchFenceOpen(line)) as LineTest }),
  Object.freeze({ kind: LineKind.INDENT_CODE, test: ((line) => (/^(?: {4}|\t)/.test(line) ? {} : null)) as LineTest }),
  ...
  Object.freeze({ kind: LineKind.MATH_BRACKET, test: ((line) => (MATH_BRACKET_RX.test(line) ? { rest: line.replace(MATH_BRACKET_RX, "") } : null)) as LineTest }),
  Object.freeze({ kind: LineKind.ATX, test: ((line) => { const m = ATX_RX.exec(line); return m ? { level: m[1]!.length, text: (m[2] || "").trim() } : null; }) as LineTest }),
  Object.freeze({ kind: LineKind.THEMATIC, test: ((line) => (isThematicBreak(line) ? {} : null)) as LineTest }),
  Object.freeze({ kind: LineKind.BLOCKQUOTE, test: ((line, _look, depth) => (/^ {0,3}>/.test(line) && depth < MAX_NEST ? {} : null)) as LineTest }),
  Object.freeze({ kind: LineKind.TABLE_HEADER, test: ((line, look) => (line.includes("|") && look !== undefined && isDelimiterRow(look) && /^ {0,3}\S/.test(line) ? {} : null)) as LineTest }),
  Object.freeze({ kind: LineKind.LIST_ITEM, test: ((line, _look, depth) => { if (depth >= MAX_NEST) return null; return matchListMarker(line); }) as LineTest }),
...
export function parseBlocks(lines: string[], ctx: BlockCtx, depth: number): Block[] {
  const { parseInline } = ctx;
  const blocks: Block[] = []; let i = 0;
  while (i < lines.length) {
    // i < lines.length holds, so lines[i] is a defined line (asserted where needed).
    const line = lines[i]!;
    ...
    if (cls.kind === LineKind.FENCE) {
      const fo = cls; const body: string[] = []; i++;
    ...
    if (cls.kind === LineKind.MATH_BRACKET) {
      // MATH_BRACKET always captures `rest`; assert it (the kind guarantees presence).
      const rest = cls.rest!;
    ...
    if (cls.kind === LineKind.TABLE_HEADER) {
      // TABLE_HEADER classification requires lines[i+1] to be a delimiter row, so it exists.
    ...
    if (cls.kind === LineKind.LIST_ITEM) {
      const ordered = cls.ordered, start = cls.start, items: Block[] = []; let tight = true;
      while (i < lines.length) {
