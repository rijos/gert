// THIN FACADE over the in-house markdown renderer (no npm, no build step). Owns no
// parsing/rendering itself - wires ctx and re-exports. Pipeline across render/*:
//
//   normalize -> parseBlocks (render/lines.js, the LINE_KINDS classifier)
//             -> render      (render/dom.js, structural renderer + the single
...
// TOTAL (HARD CONTRACT 3). Inline scanner in render/inline.js, URL/slug safety in
// render/url.js, math/code leaves are VanJS components MdMath / MdCode.
//
// Security F4: NO raw HTML is ever interpreted. Every node is built via
// createElement / createTextNode through ONE guarded createEl chokepoint over a
// closed per-(ns,tag) allow-list (render/dom.js), so injected HTML/<script>
// renders as literal text. URLs go through the single sanitizeUrl chokepoint
// (render/url.js); images allow ONLY inline data:image of known-safe media types;
// MathML (MdMath/smath) keeps its own closed MML allow-list; fenced code
// (MdCode/highlight) emits only inert tok-* spans. Math is native <math> MathML -
// zero third-party code in the model-output path.
//
...
export const renderMarkdown = (src: unknown) => {
  ...
  // TOTAL for ANY input. If normalization faults, source stays "" and the catch
  // yields an empty fragment - never a throw into the caller.
  let source = "";
  try {
    source = String(src ?? "").replace(/\r\n?/g, "\n").replace(/\t/g, "    ");
    // ctx carries the DOM target + the inline parser injected into the block parser
    // (render/lines.js owns no inline logic) and the structural renderer
    ...
    // whose per-module bags don't structurally unify. These are type-only boundary
    // casts: the runtime wiring is byte-identical to the original JS.
    ...
    // Belt-and-suspenders: the parser/renderer are total by design, but any
    // unforeseen fault must still degrade to the literal source rather than throw
    // into the caller's VanJS render loop (which would wedge the view).
