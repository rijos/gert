...
/// transport (rest-api.md section the ws endpoint).
///
/// <para>
...
public static class ChatWebSocketEndpoint
{
    ...
    {
        // {pid} shape guard (configuration.md section 2.5) BEFORE the socket is accepted:
        ...
        // RequestAborted never fires during graceful shutdown - link
        // ApplicationStopping so an open socket can't pin the host for the
        // whole drain timeout; the receive loop unwinds and disposal closes
        // the socket (the SPA reconnects and resumes from its cursor).
        ...
        await ReceiveLoopAsync(session, registry, linked.Token).ConfigureAwait(false);
    }
