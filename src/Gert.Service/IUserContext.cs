...
/// (auth.md section user context). Host-agnostic: <c>Gert.Api</c> implements it from
/// JWT claims (<c>HttpUserContext</c>); the service layer only ever
...
public interface IUserContext
{
    ...
    /// tools the model may call (auth.md section entitlement). Derived from the
    /// <c>gert_tools</c> claim AND the registry, or the default grant.
    /// </summary>
