...
public sealed class GVisorSandbox : IPythonSandbox
{
    ...
    public async Task<PythonSandboxResult> RunPythonAsync(string code, CancellationToken cancellationToken = default)
    {
        ...
        if (!IsAvailable(_parameters))
        {
            _logger.LogWarning("gVisor ({Runsc}) not available; returning unavailable result.", _parameters.RunscPath);
            return Unavailable();
        }
    ...
    /// </summary>
    public static bool IsAvailable(GVisorParameters parameters)
    {
        ...
        {
            if (File.Exists(Path.Combine(dir, parameters.RunscPath)))
            {
                return true;
            }
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var args = SandboxCommandBuilder.BuildRunscArgs(_parameters, containerId, bundlePath);

        var psi = new ProcessStartInfo
        {
            FileName = _parameters.RunscPath,
