...
public enum ToolType
{
    ...
    /// continues. The per-tool <c>ToolBounds.CallTimeout</c> backstop applies.
    /// </summary>
    Standard,

    /// <summary>
    /// A tool that legitimately blocks mid-turn on out-of-band input (<c>ask_user</c>) or runs
    /// a long nested flow (<c>run_sub_agent</c>). The runner exempts it from the per-tool
    /// <c>ToolBounds.CallTimeout</c> - it carries its own deadline math, and the turn's lifetime
    /// token stays the hard wall (chat-and-tools.md section Ask the user).
    /// </summary>
    Modal,
}
