...
/// interpreter in Rust, the default <see cref="Gert.Service.External.IPythonSandbox"/> backend
/// (chat-and-tools.md section sandbox). Bound from configuration section
/// <c>Gert:Sandbox:Monty</c>. All non-secret.
///
/// <para>
/// The per-run security posture (wall clock, memory, output cap) lives on the shared
/// <see cref="PythonSandboxOptions"/> so both backends read one set of limits; these options
