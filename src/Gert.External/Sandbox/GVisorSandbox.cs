...
/// <summary>
/// Real <see cref="ISandbox"/> running <c>run_python</c> in an ephemeral gVisor
/// (<c>runsc</c>) container (chat-and-tools.md § sandbox; security F5): no inbound
...
/// <see cref="SandboxCommandBuilder"/>; the failure → graceful-<see cref="SandboxResult"/>
...
/// </para>
/// </summary>
public sealed class GVisorSandbox : ISandbox
{
    private readonly SandboxOptions _options;
    private readonly ILogger<GVisorSandbox> _logger;

    /// <summary>Construct over the configured limits.</summary>
    public GVisorSandbox(IOptions<SandboxOptions> options, ILogger<GVisorSandbox> logger)
    {
    ...
    public async Task<SandboxResult> RunPythonAsync(string code, CancellationToken cancellationToken = default)
    {
        ...
        // down is validated on a gVisor host (U13 / staging), not in CI. We keep the
    ...
    /// </summary>
    public static bool IsAvailable(SandboxOptions options)
    {
    ...
    /// <summary>
    /// Map a spawn/run failure to a graceful <see cref="SandboxResult"/> — a non-zero
    ...
    private static SandboxResult Unavailable() => new()
    {
    ...
    /// <see cref="MapFailure"/>.
    /// </summary>
    private async Task<SandboxResult> RunRunscAsync(
        string containerId,
        SandboxRuntimeConfig runtime,
        CancellationToken cancellationToken)
    {
        // TODO(U13/staging): write the OCI bundle from `runtime` (rootfs, config.json
