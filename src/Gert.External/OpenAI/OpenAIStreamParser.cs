...
public sealed class OpenAIStreamParser
{
    ...
    public IReadOnlyList<ChatModelChunk> Parse(StreamingChatCompletionUpdate update)
    {
        ...
        // answer's content deltas. vLLM 0.22 renamed the output field
        // `reasoning_content` -> `reasoning`; accept both so the parser works
        // against either server generation. Spec-unknown fields ride the Patch
        // (SCME0001 is NoWarn'd project-wide; see the csproj note).
