...
/// (storage-and-data.md section chat.db). Wraps a single open connection opened by the
...
/// round-trippable ISO-8601 (<c>o</c>) UTC text. <c>tools_json</c> holds the
/// serialised <see cref="ToolToggles"/> (sampling rides the provider, not the row).
/// </para>
/// </summary>
public sealed class SqliteChatRepository(SqliteConnection connection) : IChatRepository
{
    ...
    {
        const string sql =
            "SELECT id, title, model_id, tools_json, created_at, updated_at, archived " +
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        const string sql =
            "INSERT OR IGNORE INTO conversations (id, title, model_id, tools_json, created_at, updated_at, archived) " +
            "VALUES (@Id, @Title, @ModelId, @ToolsJson, @CreatedAt, @UpdatedAt, @Archived);";
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // seq is the primary order; rows with the seq=0 default fall back to
        // created_at.
        const string sql =
