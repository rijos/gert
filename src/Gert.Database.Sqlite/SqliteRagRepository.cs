...
/// <summary>
/// Dapper-backed <see cref="IRagRepository"/> over one project's <c>rag.db</c>
/// (storage-and-data.md § rag.db). Wraps a single open connection — opened by the
...
/// (chat-and-tools.md § "memory rides the same query"). The three indexes share an
...
/// by fused score are returned (chat-and-tools.md § hybrid retrieval).
/// </para>
/// </summary>
public sealed class SqliteRagRepository : IRagRepository
{
    ...
    public SqliteRagRepository(SqliteConnection connection)
    {
