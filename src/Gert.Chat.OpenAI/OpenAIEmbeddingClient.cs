...
public sealed class OpenAIEmbeddingClient : IEmbeddingClient
{
        ...
        ILogger<OpenAIEmbeddingClient> logger)
    {
        ...
        _parameters = options?.Value.Parameters ?? throw new ArgumentNullException(nameof(options));
        _logger = logger ?? throw new ArgumentNullException(nameof(logger));
        _client = OpenAIChatModelClient.CreateSdkClient(http, _parameters.BaseUrl, _parameters.ApiKey);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return MapResponse(embeddings, texts.Count, _parameters.Dimensions);
    }
