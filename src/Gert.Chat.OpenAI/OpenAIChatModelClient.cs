...
/// bounds only the pre-stream phase, per the provider's
/// <see cref="ChatProviderParameters.RequestTimeoutSeconds"/>/<see cref="ChatProviderParameters.RetryCount"/>).
...
{
    /// <summary>
    /// The named <c>HttpClient</c> for one chat provider's OpenAI-compatible upstream.
    /// There is one client per provider slug (each carries that provider's own pre-stream
    /// resilience pipeline + wire trace + infinite timeout), so the transport for a hot
    /// conversation is the provider's, not a single shared one.
    /// </summary>
    ...
    /// Shared by the chat client (per provider) and the embeddings client (the Gert:Embeddings connection).
    /// </summary>
